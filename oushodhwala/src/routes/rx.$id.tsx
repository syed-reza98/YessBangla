...
import { getConsultationRxAction, getAppointmentAction } from "@/actions/consult";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [rx, appt] = await Promise.all([
        getConsultationRxAction(id).then(d=>({data:d,error:null})),
        getAppointmentAction(id).then(d=>({data:d,error:null})),
      ]);
