...
function CategoryPage() {
  const t = useT();
  const { lang } = useLang();
  const loaded = Route.useLoaderData() as { cat: (typeof staticCategories)[number] } | undefined;
  if (!loaded) {
    return <div className="py-16 text-center text-sm text-muted-foreground">লোড হচ্ছে…</div>;
  }
  const { cat } = loaded;
