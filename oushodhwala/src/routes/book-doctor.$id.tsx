...
import { getDoctorBlackoutDaysAction } from "@/actions/admin-catalog";
import { bookAppointmentAction, getDoctorTakenSlotsAction } from "@/actions/appointments";
...
function BookDoctor() {
  ...
  const { data: blackouts = [] } = useQuery({
    ...
    queryFn: async () => {
      return await getDoctorBlackoutDaysAction(id);
    },
  });
  ...
  const { data: taken = [] } = useQuery({
    ...
    queryFn: async () => {
      const result = await getDoctorTakenSlotsAction({
        doctorId: id,
        from: from.toISOString(),
        to: to.toISOString(),
      });
      if (!result.ok) throw new Error(result.error);
      return result.slots.map((s) => new Date(s).getTime());
    },
  });
  ...
  const book = useMutation({
    mutationFn: async () => {
      ...
      const result = await bookAppointmentAction({
        doctorId: id,
        mode,
        scheduledAt: slotDate(day, time).toISOString(),
      ...
      if (!result.ok) {
        opsFailure("appointment_booking", new Error(result.error), { doctorId: id, mode });
        if (result.error.includes("SLOT_TAKEN")) throw new Error(t("এই সময়টি ইতিমধ্যে বুক হয়ে গেছে, অন্য সময় নিন", "This slot is already booked, please choose another time"));
        if (result.error.includes("PAST_SLOT")) throw new Error(t("অতীতের সময় নির্বাচন করা যাবে না", "You cannot select a past time"));
        throw new Error(result.error);
      }
      return { id: result.id, invoice_no: result.invoice_no };
    },
    onSuccess: (appt) => {
