...
import { getAccountCountsAction } from "@/actions/admin-entities";
...
function Account() {
  ...
  const { data: orderCount } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const counts = await getAccountCountsAction();
      const count = counts.ok ? counts.orders : 0;
      return count ?? 0;
    },
  });
  ...
  const { data: apptCount } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const counts2 = await getAccountCountsAction();
      const count = counts2.ok ? counts2.appointments : 0;
      return count ?? 0;
    },
  });
