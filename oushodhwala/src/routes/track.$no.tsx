...
import { trackOrderPageAction } from "@/actions/delivery-query";
...
function Track() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const res = await trackOrderPageAction(no);
      if (!res.ok) throw new Error(res.error);
      return { order: res.order, delivery: res.delivery, events: res.events, notifications: res.notifications };
    },
  });
