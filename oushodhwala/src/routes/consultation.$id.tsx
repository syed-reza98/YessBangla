...
import {
  getAppointmentAction, getConsultationRxAction, upsertConsultationRxAction,
  insertConsultationMessageAction, insertConsultationMediaAction,
  deleteConsultationMediaAction, upsertDoctorReviewAction, setAppointmentStatusAction,
  listConsultationMediaAction,
...
function ConsultationRoom() {
  ...
  const { data: appt, isLoading } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const { data, error } = await getAppointmentAction(id).then(d=>({data:d,error:null}));
  ...
  const { data: media = [] } = useQuery({
    ...
    enabled: !!appt,
    queryFn: async () => {
      return await listConsultationMediaAction(id);
    },
  });
...
function CancelBox({
  ...
  const cancel = useMutation({
    mutationFn: async () => {
      ...
      const result = await cancelAppointmentAction({ appointmentId: appt.id, reason: reason.trim() });
      if (!result.ok) {
        opsFailure("appointment_cancel", new Error(result.error), { appointmentId: appt.id });
...
function RxBox({
  ...
  const { data: rx } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await getConsultationRxAction(appointmentId).then(d=>({data:d,error:null}));
      if (error) throw error;
      return data;
    },
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const _rx = await upsertConsultationRxAction({ ...payload, existing: !!rx } as any); const error = _rx.ok ? null : { message: _rx.error };
      if (error) {
...
function ChatBox({
  ...
  const send = useMutation({
    mutationFn: async (file?: File) => {
      ...
      const { error } = await insertConsultationMessageAction({
        appointment_id: appointmentId,
        user_id: userId,
        sender: "patient",
        body: text.trim(),
...
function MediaBox({
  ...
  const save = useMutation({
    mutationFn: async (file?: File) => {
      ...
      const { error } = await insertConsultationMediaAction({
        appointment_id: appointmentId,
        user_id: userId,
  ...
  const del = useMutation({
    mutationFn: async (mid: string) => {
      const { error } = await deleteConsultationMediaAction(mid).then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
...
function ReviewBox({
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = review
        ? await upsertDoctorReviewAction({ ...payload, appointment_id: appointmentId, existing: true } as any)
        : await upsertDoctorReviewAction({ ...payload, appointment_id: appointmentId, existing: false } as any);
      if (error) throw error;
      await setAppointmentStatusAction(appointmentId, "completed");
    },
    onSuccess: () => {
