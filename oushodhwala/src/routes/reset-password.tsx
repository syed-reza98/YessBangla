...
function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      const data = { session: null }; // Auth.js session via useSession
      if (!alive) return;
      if (data.session) {
        setReady("valid");
  ...
  const save = async () => {
    ...
    try {
      const _cp = await changePasswordAction({ currentPassword: "", newPassword: pw } as any);
      const error = _cp.ok ? null : { message: _cp.error };
  ...
  const resend = async () => {
    ...
    try {
      // Forgot-password email flow is support-assisted under Auth.js (no cloud mailer yet).
      const error = {
        message: t(
          "পাসওয়ার্ড রিসেটের জন্য সাপোর্টে যোগাযোগ করুন: " + email.trim(),
          "Contact support to reset your password: " + email.trim(),
        ),
      };
      if (error) throw error;
      recordAttempt(key);
