...
import { uploadPrescriptionAction } from "@/actions/clinical";
...
function Prescription() {
  ...
  const uploadOne = async (p: Picked, folder: string) => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      await uploadFile("prescriptions", path, p.file);
      const error = null;
  ...
  const submit = useMutation({
    mutationFn: async (uidArg?: string) => {
      ...
      setPhase("upload");
      setDone(0);
      if (picked.length === 0) throw new Error(t("ফাইল নির্বাচন করুন", "Select a file"));
      let lastId = "";
      const bad: string[] = [];
      for (const p of picked) {
        try {
          ...
          const result = await uploadPrescriptionAction(fd);
      ...
      setPhase("save");
      opsSuccess("prescription_upload", "", { files: picked.length });
      if (!uid && lastId) rememberGuestRx(lastId);
      setPhase("read");
      return lastId;
    },
