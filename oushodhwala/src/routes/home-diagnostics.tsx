...
import { bookHomeDiagnosticAction } from "@/actions/clinical";
...
function HomeDiagnostics() {
  ...
  const submit = async () => {
    ...
    const result = await bookHomeDiagnosticAction({
    ...
    setBusy(false);
    if (!result.ok) {
      opsFailure("diagnostic_booking", new Error(result.error), { tests: chosen.length });
      setErr(result.error);
      return;
    }
    const total = chosen.reduce((s, x) => s + Number(x.price), 0) + COLLECTION_FEE;
    const bookingNo = result.id.slice(0, 8).toUpperCase();
    opsSuccess("diagnostic_booking", bookingNo, { total });
    setDone({ no: bookingNo, total });
    setPicked([]);
    void refetch();
  };
