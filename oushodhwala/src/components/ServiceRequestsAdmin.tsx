...
import { listServiceRequestsAction } from "@/actions/admin-entities";
import { bn } from "@/data/catalog";
import { adminSetServiceStatusAction } from "@/actions/admin-rpc";
...
export function ServiceRequestsAdmin() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      const _sr = await listServiceRequestsAction(200);
      if (!_sr.ok) throw new Error(_sr.error);
      let query = { data: _sr.data, error: null } as any;
      const { data, error } = { data: _sr.data, error: null };
  ...
  const setStat = useMutation({
    mutationFn: async (v: { id: string; status: string; name?: string; phone?: string }) => {
      const res = await adminSetServiceStatusAction({
        requestId: v.id,
        status: v.status,
        assigneeName: v.name ?? "",
        assigneePhone: v.phone ?? "",
        adminNote: "",
      });
