...
import { updatePrescriptionStatusAction } from "@/actions/admin-entities";
...
export function RxAdmin() {
  ...
  const update = useMutation({
    mutationFn: async ({ id, status, admin_note }: { id: string; status: string; admin_note: string }) => {
      const { error } = await updatePrescriptionStatusAction(id, status, admin_note).then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const openFile = async (path: string) => {
    const url = await resolveFileUrl("prescriptions", path);
    const data = { signedUrl: url }; const error = url ? null : { message: "missing" };
    if (error || !data) {
