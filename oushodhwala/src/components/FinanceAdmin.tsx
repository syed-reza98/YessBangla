...
export function ExpensesAdmin() {
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await deleteExpenseAction(id).then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
...
export function ChartOfAccounts() {
  ...
  const { data } = useQuery({
    queryKey: ["coa"],
    queryFn: async () => {
      const { data, error } = await listChartAccountsAction().then(r=>({data:r.ok?r.data:null,error:r.ok?null:{message:r.error}}));
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const add = useMutation({
    mutationFn: async () => {
      const { error } = await insertChartAccountAction(f).then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
    onSuccess: () => {
...
export function DayBook() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await dayBookAction({ day });
      if (!result.ok) throw new Error(result.error);
      // Map journal entries into the DayBook panels (orders/pos/expenses stay empty until expanded)
      return {
        orders: [],
        pos: [],
        expenses: [],
        journals: result.entries,
      } as unknown as DayBookData;
    },
  });
...
export function Financials() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await financeSummaryAction({ from, to });
      if (!result.ok) throw new Error(result.error);
      ...
      return {
        online_sales: byType("revenue") || byType("income"),
        pos_sales: 0,
        purchases: byType("expense"),
...
export function PartyStatement() {
  ...
  const { data: suppliers } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await listSuppliersAction().then(r=>({data:r.ok?r.data:null,error:r.ok?null:{message:r.error}}));
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const { data: customers } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminListCustomersAction({ q: "", limit: 200 });
      if (!res.ok) throw new Error(res.error);
      return res.customers as { user_id: string; name: string }[];
    },
  });
  ...
  const { data: stmt, isFetching } = useQuery({
    ...
    enabled: !!party,
    queryFn: async () => {
      const result = await partyStatementAction({
        kind,
        partyId: party,
        from: new Date(Date.now() - 365 * 864e5).toISOString().slice(0, 10),
        to: today(),
      });
      if (!result.ok) throw new Error(result.error);
      return {
        rows: (result.rows as Array<Record<string, unknown>>).map((r) => ({
          ref: String(r.entityId || r.id || ""),
          detail: String(r.details || r.action || ""),
          debit: 0,
          credit: 0,
