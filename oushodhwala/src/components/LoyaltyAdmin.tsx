...
import {
  adminAdjustLoyaltyAction,
  listLoyaltyAccountsAction,
...
export function LoyaltyAdmin() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await listLoyaltyAccountsAction(200);
      if (!result.ok) throw new Error(result.error);
      return result.data.map((r) => {
        const balance = Number(r.points || 0);
        return {
          user_id: r.userId,
  ...
  const adjust = useMutation({
    mutationFn: async (v: { id: string; pts: number; reason: string }) => {
      const result = await adminAdjustLoyaltyAction({
        userId: v.id,
        delta: v.pts,
        note: v.reason,
      });
