...
import { adminSetRefundStatusAction } from "@/actions/admin-rpc";
import {
  MODE_LABEL,
  REFUND_LABEL,
  STATUS_LABEL,
...
export function Consultations() {
  ...
  const { data: list = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await listAppointmentsAdminAction().then(r=>({data:r.ok?r.data:null,error:r.ok?null:{message:r.error}}));
  ...
  const refund = useMutation({
    mutationFn: async ({ id, st }: { id: string; st: string }) => {
      const res = await adminSetRefundStatusAction({ appointmentId: id, status: st });
  ...
  const complete = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await setAppointmentStatusAction(id, "completed").then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [media, review] = await Promise.all([
        listConsultationMediaAction(appointmentId).then(d=>({data:d,error:null})),
