...
export function ApiHub() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await listApiEndpointsAction().then(r=>({data:r.ok?r.data:null,error:r.ok?null:{message:r.error}}));
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const res = await upsertApiEndpointAction(editId ? { ...payload, id: editId } : payload);
      if (!res.ok) throw new Error(res.error);
    },
    onSuccess: () => {
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await deleteApiEndpointAction(id).then(r=>({error:r.ok?null:{message:r.error}}));
      if (error) throw error;
    },
    onSuccess: () => {
