...
import {
  listStockCountsAction, listStockCountItemsAction,
  insertStockCountItemAction, updateStockCountItemQtyAction,
...
export function StockAdjustments() {
  ...
  const apply = useMutation({
    mutationFn: async () => {
      const res = await applyStockAdjustmentAction({
        reason,
        note,
        items: items.map((i) => ({ product_id: i.product_id, change: Number(i.change) || 0, note: "" })),
      });
...
export function StockCount() {
  ...
  const { data: counts } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await listStockCountsAction(20).then(r=>{ if(!r.ok) throw new Error(r.error); return {data:r.data,error:null}; });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const { data: rows } = useQuery({
    ...
    enabled: !!activeId,
    queryFn: async () => {
      const { data, error } = await listStockCountItemsAction(activeId!).then(r=>{ if(!r.ok) throw new Error(r.error); return {data:r.data,error:null}; });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const addItem = useMutation({
    mutationFn: async (p: P) => {
      const _sci = await insertStockCountItemAction({
        count_id: activeId!,
        product_id: p.id,
        product_name: p.name,
        system_qty: p.stock,
        counted_qty: p.stock,
      }); if (!_sci.ok) throw new Error(_sci.error);
    },
  ...
  const setCounted = useMutation({
    mutationFn: async (v: { id: string; qty: number }) => {
      const _scu = await updateStockCountItemQtyAction(v.id, v.qty); if (!_scu.ok) throw new Error(_scu.error);
    },
    onSuccess: () => void qc.invalidateQueries({ queryKey: ["stock-count-items", activeId] }),
  });
  ...
  const applyCount = useMutation({
    mutationFn: async () => {
      const res = await applyStockCountAction(activeId!);
      if (!res.ok) throw new Error(res.error);
