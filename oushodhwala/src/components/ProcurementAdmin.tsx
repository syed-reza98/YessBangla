...
import {
  listSuppliersAction, insertSupplierAction, setSupplierActiveAction,
} from "@/actions/admin-entities";
import { searchProductsLiteAction } from "@/actions/admin-catalog";
import {
  adminAdjustStockAction,
  adminCreatePurchaseOrderAction,
  adminExpiringBatchesAction,
...
export function SuppliersAdmin() {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const res = await insertSupplierAction({ ...form, name: form.name.trim() });
  ...
  const toggle = useMutation({
    mutationFn: async ({ id, active }: { id: string; active: boolean }) => {
      const res = await setSupplierActiveAction(id, active);
      if (!res.ok) throw new Error(res.error);
    },
...
export function PurchaseOrdersAdmin() {
  ...
  const { data: suppliers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await listSuppliersAction(true).then(r=>({data:r.ok?r.data:null,error:r.ok?null:{message:r.error}}));
      if (error) throw error;
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const res = await adminCreatePurchaseOrderAction({
        supplierId,
        items,
  ...
  const receive = useMutation({
    mutationFn: async (id: string) => {
      const res = await adminReceivePurchaseOrderAction({ poId: id });
      if (!res.ok) throw new Error(res.error);
    },
    onSuccess: () => {
...
export function BatchesAdmin() {
  ...
  const { data: expiring = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminExpiringBatchesAction({ days });
      if (!res.ok) throw new Error(res.error);
      return (res.batches ?? []) as { id: string; product_name: string; batch_no: string; expiry: string; qty: number; days_left: number }[];
    },
  });
  ...
  const { data: found = [] } = useQuery({
    ...
    enabled: q.trim().length > 1,
    queryFn: async () => {
      const res = await searchProductsLiteAction(q, 8);
  ...
  const adjust = useMutation({
    mutationFn: async () => {
      ...
      const res = await adminAdjustStockAction({
