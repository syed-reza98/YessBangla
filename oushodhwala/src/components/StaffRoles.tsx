...
export function StaffRoles() {
  ...
  const { data: staff = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminListStaffAction();
  ...
  const { data: found = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminListCustomersAction({ q: q.trim(), limit: 8 });
      if (!res.ok) throw new Error(res.error);
      return res.customers as FoundRow[];
    },
  });

  const setRole = useMutation({
    mutationFn: async ({ id, role, grant }: { id: string; role: AppRole; grant: boolean }) => {
      const res = await adminSetRoleAction({ userId: id, role, grant });
      if (!res.ok) throw new Error(res.error);
    },
    onSuccess: () => {
