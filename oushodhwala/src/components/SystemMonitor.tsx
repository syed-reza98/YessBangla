...
import {
  ...
  adminListCustomersAction,
  adminListErpUsersAction,
  adminSetErpManagerAction,
  adminSystemStatsAction,
  runStockAlertsAction,
...
export function SystemMonitor() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminSystemStatsAction();
      if (!res.ok) throw new Error(res.error);
      return (res.stats ?? {}) as Stats;
    },
  ...
  const runAlerts = useMutation({
    mutationFn: async () => {
      const res = await runStockAlertsAction({ expiryDays: 60 });
      if (!res.ok) throw new Error(res.error);
      return res.count;
    },
    onSuccess: (n) => {
...
export function ErpAudit() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const _al = await listErpAuditLogAction(300);
      const data = _al.ok ? _al.data : [];
      const error = _al.ok ? null : { message: _al.error };
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const { data: people = [] } = useQuery({
    ...
    queryFn: async () => {
      const _pn = await listProfileNamesAction();
      const data = _pn.ok ? _pn.data : [];
      const error = _pn.ok ? null : { message: _pn.error };
      if (error) throw error;
...
export function ErpReports() {
  ...
  const loadBatches = async () => {
    const res = await adminExpiringBatchesAction({ days: 365 });
    if (!res.ok) throw new Error(res.error);
    return (res.batches ?? []).map((r) => ({
      product_name: r.product_name,
      batch_no: r.batch_no || "—",
      expiry: r.expiry,
...
export function ErpRoles() {
  ...
  const { data: erpUsers = [] } = useQuery({
    queryKey: ["erp-users"],
    queryFn: async () => {
      const res = await adminListErpUsersAction();
      if (!res.ok) throw new Error(res.error);
      return res.users ?? [];
    },
  });
  ...
  const { data: found = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminListCustomersAction({ q, limit: 8 });
      if (!res.ok) throw new Error(res.error);
  ...
  const setRole = useMutation({
    mutationFn: async ({ id, grant }: { id: string; grant: boolean }) => {
      const res = await adminSetErpManagerAction({ userId: id, grant });
      if (!res.ok) throw new Error(res.error);
    },
    onSuccess: () => {
