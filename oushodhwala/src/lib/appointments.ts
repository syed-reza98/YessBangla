import { uploadConsultationFileAction } from "@/actions/consult";
...
export async function uploadConsultFile(_userId: string, appointmentId: string, file: File) {
  const fd = new FormData();
  fd.set("file", file);
  fd.set("appointmentId", appointmentId);
  const res = await uploadConsultationFileAction(fd);
  if (!res.ok) throw new Error(res.error);
  return { path: res.url!, name: file.name };
}

export async function openConsultFile(filePath: string) {
  const url = await resolveFileUrl(CONSULT_BUCKET, filePath);
