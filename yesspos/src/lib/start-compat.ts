/**
 * TanStack Start compatibility shims for Next.js.
 * Server handlers should use Drizzle / Server Actions — no supabase context.
 */
export function createServerFn(_options?: any) {
  ...
  const builder = {
    ...
    handler<T = any>(fn: (ctx: { data: any; context: Record<string, any> }) => T) {
      _handler = fn;
